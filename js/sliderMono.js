...
let imagensMono = document.querySelectorAll('.slide-mono');
let imagemAtualMono = 0;

function trocarImagem() {
  // Remove a classe ativa da imagem atual
  imagensMono[imagemAtualMono].classList.remove('ativo');

  // Passa para a próxima imagem (volta para a primeira se for a última)
  imagemAtualMono = (imagemAtualMono + 1) % imagensMono.length;

  // Adiciona a classe ativa na nova imagem
  imagensMono[imagemAtualMono].classList.add('ativo');
}
